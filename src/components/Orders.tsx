...
export default function Orders() {
  ...
  useEffect(() => {
    const getAllOrders = async () => {
      try {
        const response = await axios.get("https://api.replicacopyindustries.com/api/orders/all", {
          headers: {
 ...
 const handleDelete = (id: string) => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(`https://api.replicacopyindustries.com/api/orders/${userIdToDelete}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("rci-token")}`,
        },
      });

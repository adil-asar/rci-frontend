...
import { Moon, SunMoon, UserLock } from 'lucide-react';
        ...
        <>
            {location?.pathname !== '/admin' && (
                <div className="relative max-w-7xl mx-auto py-3 px-2" >
                    ...
                    <div className={`${!menuOpen ? 'hidden' : 'block'} fixed inset-0 bg-zinc-900 h-screen py-3 px-2 z-50`} >
                        ...
                        <ul className='flex flex-col items-center w-full cursor-pointer uppercase will-change-transform' >
                            {navbar && navbar.map((item, index) => (
                                <li
                                    key={index}
                                    className={'transition-all duration-200 ease-linear text-zinc-400 inline-block hover:text-zinc-900 hover:bg-zinc-400 rotate-0 hover:-rotate-2 text-3xl sm:text-6xl p-2 font-extrabold m-2 whitespace-nowrap'}
                    ...
                    <div className='flex justify-between items-center mb-10 px-3 sm:px-10' >
                        ...
                        <div>
                            <ReactLink to={'/'} ><img src={rciLogo} alt={'rciLogo'} className='w-[10rem] cursor-pointer' /></ReactLink>
                        </div>
                        <div
                        ...
                        <div className='flex items-center justify-center gap-3' >
                            ...
                            {theme === 'light' ? (
                                <Moon strokeWidth={1.2} className='not-dark:text-zinc-900 dark:text-zinc-400 cursor-pointer' size={25} onClick={handleLightDarkMode}/>
                            ) : (
                                <SunMoon strokeWidth={1.2} className='not-dark:text-zinc-900 dark:text-zinc-400 cursor-pointer' size={25} onClick={handleLightDarkMode} />
                            )}
                            {token && user.role === 'admin' && (
                                <ReactLink to="/admin" className="text-white">
                                    <UserLock strokeWidth={1.2} className='not-dark:text-zinc-900 dark:text-zinc-400 cursor-pointer' size={25} />
                                </ReactLink>
                            )}
                            {token && user && user?.picture &&
                                <DropdownMenu>
                                    <DropdownMenuTrigger>
                                        <Avatar className='cursor-pointer'>
                                            <AvatarImage src={user?.picture} alt='GN' />
                    ...
                    {location?.pathname === '/' && (
                        <div className='hidden md:flex items-center justify-center py-5' >
                            <ul className='flex items-center gap-5 lg:gap-10 text-sm font-medium cursor-pointer uppercase justify-center w-full' >
                                {navbar && navbar.map((item, index) => (
                                    <li
                                        className={`${active === index ? 'text-zinc-400' : 'text-zinc-500'} hover:text-zinc-400`}
                                    >
                                        <Link
                                            to={item.to}
                                            smooth={true}
